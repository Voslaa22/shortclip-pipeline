...
REQUIRED_FIELDS_SEGMENTS = {"segments", "title", "hook"}
...
def main():
    ...
    def span_start(c):
        return c["start"] if "start" in c else min(s["start"] for s in c["segments"])

    sorted_clips = sorted(enumerate(clips), key=lambda c: span_start(c[1]))
    prev_end = None
    prev_idx = None

    for idx, clip in sorted_clips:
        is_multi = "segments" in clip
        missing = (REQUIRED_FIELDS_SEGMENTS if is_multi else REQUIRED_FIELDS) - clip.keys()
        if missing:
            problems.append(f"Clip #{idx}: missing fields {missing}")
            continue

        segments = clip["segments"] if is_multi else [{"start": clip["start"], "end": clip["end"]}]
        if is_multi and (not isinstance(segments, list) or not segments):
            problems.append(f"Clip #{idx} '{clip['title']}': segments must be a non-empty list.")
            continue

        bad = False
        for seg in segments:
            if not isinstance(seg, dict) or "start" not in seg or "end" not in seg:
                problems.append(f"Clip #{idx} '{clip['title']}': each segment needs start/end.")
                bad = True
                break
            s, e = seg["start"], seg["end"]
            if not isinstance(s, (int, float)) or not isinstance(e, (int, float)):
                problems.append(f"Clip #{idx} '{clip['title']}': segment start/end must be numbers.")
                bad = True
                break
            if e <= s:
                problems.append(f"Clip #{idx} '{clip['title']}': segment end ({e}) must be after start ({s}).")
                bad = True
                break
            if video_duration and e > video_duration:
                problems.append(
                    f"Clip #{idx} '{clip['title']}': segment end ({e}s) is past the video's "
        ...
        if dur < MIN_CLIP_SECONDS or dur > MAX_CLIP_SECONDS:
            warnings.append(
                f"Clip #{idx} '{clip['title']}': assembled duration {dur:.1f}s is outside the "
